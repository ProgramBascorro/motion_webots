...
class Field:
    def __init__(self, params: Optional[Dict[str, float]] = None, logger=None):
        ...
        self.distance_point_threshold = self._param("distance_point_threshold", 5.0)
        self.min_points_threshold = self._param("min_points_threshold", 40)
        self.max_detected_line_parallel_offset_error = self._param("max_detected_line_parallel_offset_error", 0.1)
        self.max_detected_line_perpendicular_offset_error = self._param("max_detected_line_perpendicular_offset_error", 0.3)
        self.offset_movement_limit = self._param("offset_movement_limit", 0.2)

        self.max_detected_line_parallel_offset_error_localizing = self._param("max_detected_line_parallel_offset_error_localizing", 0.5)
        self.max_detected_line_perpendicular_offset_error_localizing = self._param(
            "max_detected_line_perpendicular_offset_error_localizing", 0.5
        )
        self.offset_movement_limit_localizing = self._param("offset_movement_limit_localizing", 0.5)

        self.path_plots: Dict[str, object] = {}
        self.path_points: Dict[str, list] = {}

    def _param(self, name: str, default):
        return self._params.get(name, default)

    def _log(self, level: str, message: str, throttle_s: Optional[float] = None, key: Optional[str] = None):
        if not self._logger:
            return
        if throttle_s is not None:
            now = time.time()
            log_key = key or message
            last = self._last_log.get(log_key, 0.0)
    ...
    def matchPointsWithMap(
        ...
        self._log("debug", f"Match Points with Map rate (s): {end - start}", throttle_s=1.0, key="match_rate")
        self._log(
            "debug",
            f"Transform Confidence: {transform_confidence} Ratio Meet Distance Threshold: {ratio_meets_distance_threshold}",
            throttle_s=1.0,
