...
# Stock opencr_op3 wiring: the OpenCR serves sub controller ID 200 (IMU, button,
# buzzer, DXL power) on DXL_PORT = Serial3, i.e. the same TTL bus as the servos,
# so one port carries everything. Confirmed by ping -- ID 200 answers on
# /dev/ttyOP3 (model 0x7400, fw 2), while the OpenCR's micro-USB CDC
# (/dev/ttyACM0) is only a debug console and never replies to a DXL packet.
