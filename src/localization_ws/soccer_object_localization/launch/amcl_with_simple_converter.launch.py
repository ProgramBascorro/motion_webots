#!/usr/bin/env python3
"""
Complete Localization Pipeline - Using Simple PC2Scan Converter
Includes: Detector + Simple Converter + AMCL (no odometry)
"""
...
def generate_launch_description():
    """Generate complete localization pipeline with simple converter"""
    ...
    # ===== NODE 4: Simple PointCloud to LaserScan Converter =====
    # Using our proven custom converter instead of buggy pointcloud_to_laserscan
    simple_pc2scan_node = Node(
        ...
        output='screen',
        parameters=[{
            # Angle range - full 360 degrees
            'angle_min': -3.14159,
            'angle_max': 3.14159,
            'angle_increment': 0.0174533,  # 1 degree
            
            # Range limits - match detector output
            'range_min': 0.3,
            'range_max': 5.0,
    ...
    # ===== NODE 7: AMCL (Modified for cam_link base frame) =====
    amcl_node = Node(
        package='nav2_amcl',
        executable='amcl',
        name='amcl',
        output='screen',
        parameters=[{
            # TF frames - CRITICAL: use cam_link as base!
            'odom_frame_id': 'odom',
            'base_frame_id': 'cam_link',  # Match scan frame from simple_pc2scan
            'global_frame_id': 'map',
    ...
    return LaunchDescription([
        ...
        detector_node,
        simple_pc2scan_node,  # Using simple converter instead of pointcloud_to_laserscan
        map_server_node,
