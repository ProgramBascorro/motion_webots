...
const ACTIVE_COLOR = "var(--accent-2)";
const INACTIVE_COLOR = "#eadfce";
const BODY_COLOR = "#b6aa97";
...
function isJoyActive(joyData) {
  ...
  if (Array.isArray(joyData.buttons)) {
    for (const value of joyData.buttons) {
      if (Number(value) > 0.5) {
        return true;
      }
    }
  }
  if (Array.isArray(joyData.axes)) {
    for (const value of joyData.axes) {
      if (Math.abs(Number(value) || 0) > DEADZONE) {
        return true;
      }
    }
  }
...
function buildAxes(virtualAxes) {
  ...
  axes[AXIS_MAP.rx] = rx;
  axes[AXIS_MAP.ry] = ry;
  if (AXIS_MAP.rx_alt >= 0) {
    axes[AXIS_MAP.rx_alt] = rx;
  }
  if (AXIS_MAP.ry_alt >= 0) {
    axes[AXIS_MAP.ry_alt] = ry;
  }
  return axes;
}
...
export default function GamepadVisualizer({
  ...
  const [virtualButtons, setVirtualButtons] = useState(
    () => new Array(BUTTON_COUNT).fill(false)
  );
  const [virtualAxes, setVirtualAxes] = useState({
    lx: 0,
    ly: 0,
    rx: 0,
    ry: 0,
  ...
  const browserPadRef = useRef(null);
    ...
    const poll = () => {
      ...
      const next = {
        ...
        axes: Array.isArray(active.axes)
          ? active.axes.map(normalizeAxis)
          : [],
        buttons: normalizeButtons(active.buttons),
      };
      browserPadRef.current = next;
      setBrowserPad(next);
    };
    poll();
  ...
  const ensureVirtualEnabled = useCallback(() => {
    if (!virtualEnabled) {
      if (!rosConnected) {
        setNotice("Connect ROS to publish virtual /joy.");
        return false;
      }
      setNotice("");
  ...
  const browserJoy = browserPad
    ? { axes: browserPad.axes || [], buttons: browserPad.buttons || [] }
    : null;
  ...
  const handleButtonDown = useCallback(
    ...
    [emitJoy, ensureVirtualEnabled, virtualAxes]
  );
  ...
  const getAxis = useCallback(
    (index) => {
      if (!displayJoy || !Array.isArray(displayJoy.axes)) {
        return 0;
      }
      const value = displayJoy.axes[index];
      return Number.isFinite(value) ? value : 0;
    },
    [displayJoy]
  );

  const getSmartAxis = useCallback(
    (primaryIndex, altIndex, virtualValue, isDragging) => {
      if (isDragging) {
        return virtualValue;
      }
      if (Math.abs(virtualValue) > 0.01) {
        return virtualValue;
      }
      const stdVal = getAxis(primaryIndex);
      const altVal = altIndex >= 0 ? getAxis(altIndex) : 0;
      if (altIndex >= 0 && Math.abs(altVal) > DEADZONE) {
        return altVal;
      }
      if (Math.abs(stdVal) > DEADZONE) {
        return stdVal;
      }
      return 0;
    },
  ...
  const leftStickX = getSmartAxis(
    AXIS_MAP.lx,
    -1,
    virtualAxes.lx,
    draggingStick === "left"
  );
  const leftStickY = getSmartAxis(
    AXIS_MAP.ly,
    -1,
    virtualAxes.ly,
    draggingStick === "left"
  );
  const rightStickX = getSmartAxis(
    AXIS_MAP.rx,
    AXIS_MAP.rx_alt,
    virtualAxes.rx,
    draggingStick === "right"
  );
  const rightStickY = getSmartAxis(
    AXIS_MAP.ry,
    AXIS_MAP.ry_alt,
    virtualAxes.ry,
    draggingStick === "right"
  ...
  const handleStickStart = useCallback(
    (stick, event) => {
      ...
      setDraggingStick(stick);
    },
    [ensureVirtualEnabled]
  );

  const handleDragMove = useCallback(
    (event) => {
      if (!draggingStick || !svgRef.current) {
        return;
      }
      if (event.cancelable) {
        event.preventDefault();
      ...
      const center =
        draggingStick === "left" ? LEFT_STICK_CENTER : RIGHT_STICK_CENTER;
      ...
      setVirtualAxes((prev) => ({
        ...prev,
        [draggingStick === "left" ? "lx" : "rx"]: nx,
        [draggingStick === "left" ? "ly" : "ry"]: ny,
      }));
    },
    [draggingStick]
  );
  ...
  const isPressed = useCallback(
    (index) => {
      const physical =
        displayJoy && Array.isArray(displayJoy.buttons)
          ? displayJoy.buttons[index]
          : 0;
      return Boolean(physical) || virtualButtons[index];
    },
    [displayJoy, virtualButtons]
  );
  ...
  const InteractiveButton = ({ index, children, className }) =>
    cloneElement(children, {
      ...
      className: `gamepad-hit ${className || ""}`.trim(),
      style: { ...children.props.style, cursor: "pointer" },
    });

  const axes = displayJoy?.axes || [];
  const pressedButtons = displayJoy?.buttons
    ? displayJoy.buttons
        .map((value, index) => (Number(value) > 0.5 ? index : null))
    ...
    <div className="gamepad-card">
      <div className="gamepad-header">
        <div>
          <h3>Gamepad</h3>
          <p className="hint">
            Visual + clickable. Virtual /joy publishes to ROS for quick tests.
          </p>
        </div>
        <div className="gamepad-controls">
          <label className="gamepad-source">
            View
            <select
              value={visualSource}
          ...
          <label className="gamepad-toggle">
            <input
              ...
              onChange={(event) => {
                if (!rosConnected && event.target.checked) {
                  setNotice("Connect ROS to publish virtual /joy.");
                  return;
                }
                setNotice("");
                setVirtualEnabled(event.target.checked);
              }}
            />
            Virtual /joy
          </label>
      ...
      <div className="gamepad-status">
        <span className={`status-pill ${joy ? "connected" : "warn"}`}>
          ROS /joy: {joy ? (rosActive ? "active" : "idle") : "none"}
        </span>
        <span className={`status-pill ${browserPad ? "connected" : "warn"}`}>
          Browser: {browserPad ? "connected" : "none"}
        </span>
      </div>

      <div className="gamepad-shell">
        <svg
          ref={svgRef}
        ...
        >
          ...
          <InteractiveButton index={BUTTON_MAP.L2}>
            <path
              d="M180 40 Q180 10 210 10 H330 Q360 10 360 40 V60 H180 V40 Z"
              fill={isPressed(BUTTON_MAP.L2) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>
          <InteractiveButton index={BUTTON_MAP.R2}>
            <path
              d="M920 40 Q920 10 950 10 H1070 Q1100 10 1100 40 V60 H920 V40 Z"
              fill={isPressed(BUTTON_MAP.R2) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>
          <InteractiveButton index={BUTTON_MAP.L1}>
            <rect
              ...
              height="40"
              rx="10"
              fill={isPressed(BUTTON_MAP.L1) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>
          <InteractiveButton index={BUTTON_MAP.R1}>
            <rect
              ...
              height="40"
              rx="10"
              fill={isPressed(BUTTON_MAP.R1) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>

          <InteractiveButton index={BUTTON_MAP.SQUARE}>
            <circle
              cx={935.5}
              cy={283.5}
              r={47.5}
              fill={isPressed(BUTTON_MAP.SQUARE) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>
          <InteractiveButton index={BUTTON_MAP.TRIANGLE}>
            <circle
              cx={1050.5}
              cy={183.5}
              r={47.5}
              fill={isPressed(BUTTON_MAP.TRIANGLE) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>
          <InteractiveButton index={BUTTON_MAP.CROSS}>
            <circle
              cx={1050.5}
              cy={383.5}
              r={47.5}
              fill={isPressed(BUTTON_MAP.CROSS) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>
          <InteractiveButton index={BUTTON_MAP.CIRCLE}>
            <circle
              cx={1162.5}
              cy={283.5}
              r={47.5}
              fill={isPressed(BUTTON_MAP.CIRCLE) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>

          <InteractiveButton index={BUTTON_MAP.DPAD_UP}>
            <path
              d="M269 165h-77v56c9.333 11.333 30 34 38 34s29.333-22.667 39-34v-56z"
              fill={isPressed(BUTTON_MAP.DPAD_UP) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>
          <InteractiveButton index={BUTTON_MAP.DPAD_DOWN}>
            <path
              d="M269 392h-77v-56c9.333-11.333 30-34 38-34s29.333 22.667 39 34v56z"
              fill={isPressed(BUTTON_MAP.DPAD_DOWN) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>
          <InteractiveButton index={BUTTON_MAP.DPAD_LEFT}>
            <path
              d="M119 240v77h56c11.333-9.333 34-30 34-38s-22.667-29.333-34-39h-56z"
              fill={isPressed(BUTTON_MAP.DPAD_LEFT) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>
          <InteractiveButton index={BUTTON_MAP.DPAD_RIGHT}>
            <path
              d="M341 240v77h-56c-11.333-9.333-34-30-34-38s22.667-29.333 34-39h56z"
              fill={isPressed(BUTTON_MAP.DPAD_RIGHT) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>

          <g
            onMouseDown={(event) => handleStickStart("left", event)}
            onTouchStart={(event) => handleStickStart("left", event)}
            className="gamepad-stick"
          >
            <circle
              cx={LEFT_STICK_CENTER.x}
              cy={LEFT_STICK_CENTER.y}
              r={93}
              fill={isPressed(BUTTON_MAP.L3) ? ACTIVE_COLOR : INACTIVE_COLOR}
              style={{
                transform: `translate(${leftStickX * STICK_RADIUS}px, ${
                  leftStickY * STICK_RADIUS
                }px)`,
                transformOrigin: `${LEFT_STICK_CENTER.x}px ${LEFT_STICK_CENTER.y}px`,
                transition: draggingStick === "left" ? "none" : "transform 0.1s ease-out",
              }}
            />
          </g>

          <g
            onMouseDown={(event) => handleStickStart("right", event)}
            onTouchStart={(event) => handleStickStart("right", event)}
            className="gamepad-stick"
          >
            <circle
              cx={RIGHT_STICK_CENTER.x}
              cy={RIGHT_STICK_CENTER.y}
              r={93}
              fill={isPressed(BUTTON_MAP.R3) ? ACTIVE_COLOR : INACTIVE_COLOR}
              style={{
                transform: `translate(${rightStickX * STICK_RADIUS}px, ${
                  rightStickY * STICK_RADIUS
                }px)`,
                transformOrigin: `${RIGHT_STICK_CENTER.x}px ${RIGHT_STICK_CENTER.y}px`,
                transition:
                  draggingStick === "right" ? "none" : "transform 0.1s ease-out",
              }}
            />
          </g>

          <InteractiveButton index={BUTTON_MAP.SELECT}>
            <path
              d="M471 262h75v47h-75z"
              fill={isPressed(BUTTON_MAP.SELECT) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>
          <InteractiveButton index={BUTTON_MAP.START}>
            <path
              d="M728 309v-49l72 23-72 26z"
              fill={isPressed(BUTTON_MAP.START) ? ACTIVE_COLOR : INACTIVE_COLOR}
            />
          </InteractiveButton>
        </svg>

        {!displayJoy && (
          <div className="gamepad-alert">
            {resolvedSource === "browser"
              ? "No browser gamepad detected."
              : "No /joy data yet."}
          </div>
        )}
        {notice ? <div className="gamepad-alert notice">{notice}</div> : null}
      </div>
      ...
      <div className="gamepad-meta">
        <div className="gamepad-row">
          ...
          <div className="gamepad-axes">
            <span className="gamepad-chip">
              {visualSource === "auto"
                ? `auto -> ${resolvedSource}`
                : resolvedSource}
            </span>
            {browserPad?.id ? (
              <span className="gamepad-chip">{browserPad.id}</span>
            ) : null}
          </div>
        </div>
        <div className="gamepad-row">
          ...
          <div className="gamepad-axes">
            {axes.length === 0
              ...
              : axes.map((value, index) => (
                  <span key={`axis-${index}`} className="gamepad-chip">
                    {index}:{formatAxis(value)}
                  </span>
                ))}
          </div>
        </div>
        <div className="gamepad-row">
          ...
          <div className="gamepad-axes">
            {pressedButtons.length === 0
              ? "none"
              : pressedButtons.map((index) => (
                  <span key={`btn-${index}`} className="gamepad-chip">
                    {index}
                  </span>
                ))}
          </div>
