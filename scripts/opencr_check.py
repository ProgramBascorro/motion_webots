...
Pakai:
    ...
    # cek OpenCR (ID 200); portnya dideteksi sendiri
    python3 scripts/opencr_check.py

    # sekalian scan semua servo + OpenCR di bus, atau paksa satu port
    python3 scripts/opencr_check.py --scan
    python3 scripts/opencr_check.py --port /dev/ttyUSB1

Catatan penting soal topologi (robot ini):
    - Adapter FTDI FT232H (0403:6014) adalah master bus TTL. Firmware standar
      opencr_op3 melayani ID 200 di DXL_PORT = Serial3, yaitu bus TTL yang sama
      dengan servo. Jadi di satu port itu servo 1-20 DAN ID 200 sama-sama
      menjawab -- persis seperti OP3.robot menuliskannya.
    - Nomor ttyUSB-nya tidak dipegang mati: adapter bisa pindah antara
      ttyUSB0/ttyUSB1, jadi port default di sini dipindai dengan urutan yang
      sama seperti detect_dynamixel_device() di op3_manager.launch.py.
    - Kalau tidak ada satu pun ID menjawab, cek dulu adapternya benar tercolok:
    ...
    - Board yang di-flash opencr_op3_usb berperilaku sebaliknya: ID 200 dilayani
      lewat micro-USB-nya (/dev/ttyACM0), dan OP3.robot harus memberi sensor
      OPEN-CR port itu.

Kalau dynamixel_sdk belum terpasang (image robot ini memang belum punya modul
Python-nya), pakai scripts/op3_ping.py: tanpa dependensi apa pun dan cukup untuk
menjawab "ada yang menjawab di bus ini atau tidak".
"""
...
try:
    ...
    sys.exit(
        ...
        "Kalau tetap tidak ada -- image robot ini memang belum memuatnya -- pakai\n"
        "alat tanpa dependensi:  python3 scripts/op3_ping.py"
    )
...
def detect_port():
    ...
    OP3.robot boleh saja menyebut ttyUSB0 sementara adapternya sedang di
    ttyUSB1; launch file menambal salinannya sebelum manager start. Alat
    diagnosa harus memindai dengan cara yang sama supaya tidak melapor "port
    tidak ada" untuk bus yang sebenarnya sehat.
    """
...
def main():
    ...
    ap.add_argument("--port", default=None, help="port DXL (default: auto-detect)")
    ...
    if args.scan:
        ...
        if not found:
            ...
            print("  -> Cek adapter tercolok (lsusb | grep 0403) dan power robot menyala.")
    ...
    if comm != COMM_SUCCESS:
        ...
        print("  Cek: (1) adapter tercolok dan port ini memang bus TTL (lsusb | grep 0403);")
        print("       (2) OpenCR bertenaga dan kabel TTL-nya masuk ke bus servo;")
        print("       (3) tidak ada proses lain memegang port (fuser %s)." % port_name)
        port.closePort()
        sys.exit(1)
