...
action_default_path() {
  echo "$WS/src/ROBOTIS-OP3/op3_action_module/data/ALPHONSE.bin"
}
...
while true; do
  ...
  case "$action" in
    ...
    # Tidak ada lagi pertanyaan mount mode / serial device / detached. Untuk
    # robot ini jawabannya cuma satu yang benar dan sudah dipakai
    # scripts/op3_docker.sh. Pilihan lama justru berbahaya: default "Cache
    # (src read-only)" membuat /ros2_ws/src read-only, sehingga action editor
    # tidak bisa menyimpan .bin sama sekali; dan memilih serial device manual
    # berujung `--device=` yang membekukan major:minor U2D2.
    "$M_DOCKER_RUN")
