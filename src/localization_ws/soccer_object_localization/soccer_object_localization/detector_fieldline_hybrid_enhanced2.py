...
CHANGELOG v2.1 (fixes applied):
- [FIX 1] Hapus simple_thresh dari combine → sumber false positive terbesar
- [FIX 2] Perbaiki angle filter logic → buang diagonal, pertahankan field lines
- [FIX 3] roi_top_cut naik 0.45 → 0.55 → gawang tidak masuk ROI
- [FIX 4] white_threshold naik 180 → 210 → noise rumput Webots berkurang
"""
...
from .filtered_dynamic_camera_pose import FilteredDynamicCameraPose
...
class DetectorFieldlineHybrid(Node):
    ...
    - No simple_thresh combine (was causing red flood)
    """
    ...
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                ('detection.white_threshold', 210),     # [FIX 4] 180 → 210: lebih strict, noise rumput berkurang
                ...
                ('detection.roi_top_cut', 0.55),        # [FIX 3] 0.45 → 0.55: potong lebih agresif, gawang hilang
        ...
        if self.use_dynamic_tf:
            self.get_logger().info("🤖 FILTERED DYNAMIC MODE: TF tracking with smoothing")
            
            # Get optional smoothing parameters
            self.declare_parameter('camera.position_alpha', 0.7)
            self.declare_parameter('camera.rotation_alpha', 0.8)
            
            position_alpha = self.get_parameter('camera.position_alpha').value
            rotation_alpha = self.get_parameter('camera.rotation_alpha').value
            
            self.camera_pose = FilteredDynamicCameraPose(
                node=self,
        ...
        self.get_logger().info("🔥 FIXED Enhanced Field Line Detector v2.1!")
        ...
        self.get_logger().info(f"Simple thresh combine: DISABLED (fix false positives)")
        self.get_logger().info("=" * 60)
    ...
    def detect_white_lines_enhanced(self, image: np.ndarray) -> np.ndarray:
        """
        FIXED Enhanced detection - v2.1
        
        Fixes dari v2.0:
        [FIX 1] Tidak combine simple_thresh - sumber false positive utama
        [FIX 2] Angle filter diperbaiki - buang diagonal bukan field line
        [FIX 3] roi_top_cut=0.55 - gawang tidak masuk ROI
        [FIX 4] white_threshold=210 - noise rumput Webots berkurang
        """
        h, w = image.shape[:2]
        ...
        # Step 2b: Exclude goal net — blob putih besar/solid bukan field line
        image_filtered = self.exclude_large_white_blobs(image_filtered)
        ...
        # Step 7: Draw detected lines dengan angle filter yang benar
        ...
        if lines is not None:
            for line in lines:
                ...
                # [FIX 2] Angle filter yang benar:
                # - Field lines = horizontal (angle < 30°) atau vertical (angle > 60°)
                # - Buang garis diagonal murni (30° - 60°) yang bukan field line
                # - Untuk dx=0 (vertikal sempurna), selalu keep
                if dx == 0:
                    # Vertikal sempurna → keep (bisa garis tiang gawang, tapi jarang)
                    ...
                    if angle < 30 or angle > 60:
                        # [FIX 2] Ini BENAR: keep horizontal (<30°) dan vertical (>60°)
                        # Sebelumnya kondisi ini sama tapi dx>0 check salah → sekarang pakai arctan2
                        cv2.line(line_mask, (x1, y1), (x2, y2), 255, self.line_thickness)
                        line_count += 1
                    else:
                        rejected_count += 1
        ...
        # Step 8: [FIX 1] TIDAK combine dengan simple_thresh
        # Sebelumnya: combined = cv2.bitwise_or(line_mask, simple_thresh)
        # simple_thresh menyebabkan seluruh frame putih karena noise rumput Webots
        # Sekarang: hanya pakai Hough lines
        combined = line_mask.copy()
        ...
        # Log rejection info setiap 200 frame
        if self.frame_count % 200 == 0 and lines is not None:
            total = line_count + rejected_count
            self.get_logger().info(
                f"[Hough] accepted={line_count} rejected_diagonal={rejected_count} "
                f"total={total}"
            )
        
        return combined
    
    def exclude_large_white_blobs(self, image: np.ndarray) -> np.ndarray:
        """
        Hapus blob putih besar dari image (gawang, tiang, net).
        Field lines = garis tipis panjang → area kecil per komponen.
        Goal net = blob solid besar → area besar → dihapus.

        Threshold: komponen dengan bounding-box area > 2% dari total frame dianggap gawang.
        """
        h, w = image.shape[:2]
        frame_area = h * w
        max_blob_area = int(frame_area * 0.02)   # 2% frame = ~2580px untuk 1280x720 (garis, bukan gawang)
        # Gawang di Webots ~ 15-25% frame area

        gray_temp = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        _, white_temp = cv2.threshold(gray_temp, self.white_threshold, 255, cv2.THRESH_BINARY)

        # Connected components analysis
        num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(white_temp, connectivity=8)

        # Buat mask: hanya keep komponen kecil (field lines), hapus komponen besar (gawang)
        ...
        # Apply mask ke image asli
        result = cv2.bitwise_and(image, image, mask=keep_mask)
        return result
    ...
    def create_roi_mask(self, h: int, w: int) -> np.ndarray:
        ...
        Removes:
        - Top portion (goal nets, horizon, sky) — [FIX 3] naik ke 55%
        - Bottom portion (robot body, noise)
        """
        roi_mask = np.zeros((h, w), dtype=np.uint8)
        
        # Calculate ROI boundaries
        top_cut = int(h * self.roi_top_cut)      # [FIX 3] default 0.55
    ...
    def log_statistics(self):
        ...
        if self.use_enhanced and len(self.hough_line_counts) > 0:
            ...
            self.get_logger().info(f"  Avg Hough lines (accepted): {avg_lines:.1f}")
