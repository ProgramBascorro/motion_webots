apply_profile() {
  ...
  case "$profile" in
    ...
    real_robot)
      ...
      # studio panes start, so they don't steal CPU/USB during the init and
      # inflate it (observed ~3.5 s -> ~8 s + big variance under contention).
  ...
  ACTION_FILE_SEED="${OP3_ACTION_FILE_SEED:-$WS/src/ROBOTIS-OP3/op3_action_module/data/ALPHONSE.bin}"
  ACTION_FILE_PATH="${OP3_ACTION_FILE:-$WS/src/ROBOTIS-OP3/op3_action_module/data/ALPHONSE.bin}"
