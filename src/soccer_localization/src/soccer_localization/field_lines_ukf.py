...
class FieldLinesUKF:
    ...
    def _regularize_covariance(self, min_eig=1e-9):
        P = 0.5 * (self.ukf.P + self.ukf.P.T)
        if not np.all(np.isfinite(P)):
            self.ukf.P = np.eye(P.shape[0]) * min_eig
            return

        min_eigval = float(np.min(np.linalg.eigvalsh(P)))
        if min_eigval < min_eig:
            P = P + np.eye(P.shape[0]) * (min_eig - min_eigval)
    ...
    def predict(self, u, dt):
        ...
        try:
            ...
            self._regularize_covariance(min_eig=1e-6)
            self.ukf.predict(dt=dt, u=u)
