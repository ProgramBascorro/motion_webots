...
class Field:
    def __init__(self, node=None):
        """
        Initialize Field with optional ROS2 node for parameter handling.
        
        Args:
            node: Optional ROS2 Node instance for parameter access and logging
        """
        self._node = node
        
        # Dimensions given here https://cdn.robocup.org/hl/wp/2021/06/V-HL21_Rules_v4.pdf

        self.distance_point_threshold = self._get_param("distance_point_threshold", 5)
        self.min_points_threshold = self._get_param("min_points_threshold", 40)
        self.max_detected_line_parallel_offset_error = self._get_param("max_detected_line_parallel_offset_error", 0.1)
        self.max_detected_line_perpendicular_offset_error = self._get_param("max_detected_line_perpendicular_offset_error", 0.3)
        self.offset_movement_limit = self._get_param("offset_movement_limit", 0.2)

        self.max_detected_line_parallel_offset_error_localizing = self._get_param("max_detected_line_parallel_offset_error_localizing", 0.5)
        self.max_detected_line_perpendicular_offset_error_localizing = self._get_param("max_detected_line_perpendicular_offset_error_localizing", 0.5)
        self.offset_movement_limit_localizing = self._get_param("offset_movement_limit_localizing", 0.5)

        self.path_plots: Dict[str, PathCollection] = {}
        self.path_points: Dict[str, list] = {}

    def _get_param(self, name: str, default_value):
        """
        Get ROS2 parameter with fallback to default value.
        
        Args:
            name: Parameter name
            default_value: Default value if parameter not found or no node available
        ...
        if self._node is not None:
            try:
                # Try to get parameter, declare if it doesn't exist
                ...
                if hasattr(self._node, 'get_logger'):
                    self._node.get_logger().warn(f"Failed to get parameter '{name}': {e}, using default: {default_value}")
                return default_value
        return default_value
    
    def _log_error(self, throttle_sec: float, message: str):
        """
        Log error message with throttling if node available.
        
        Args:
            throttle_sec: Throttle duration in seconds
            message: Message to log
        """
        if self._node is not None and hasattr(self._node, 'get_logger'):
            self._node.get_logger().error(message, throttle_duration_sec=throttle_sec)
        # If no node, silently skip logging (for testing without ROS)
    ...
    def matchPointsWithMap(
        ...
        self._log_error(60, f"Match Points with Map rate (s) :  {(end - start)}")
        self._log_error(1, f"Transform Confidence :  {transform_confidence} Ratio Meet Distance Threshold: {ratio_meets_distance_threshold}")
